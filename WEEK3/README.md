...
Authentication Flow
POST /user-api/login with { username, password } → returns a JWT in a cookie
Include the cookie in subsequent requests to access protected routes
The verifyToken middleware validates the JWT on every protected route
Error Handling
